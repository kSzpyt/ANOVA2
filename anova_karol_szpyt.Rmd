...
Spodziewać się można, że fakt czy matka w ciązy pali czy nie będzie miał statystycznie istotny wpływ na wagę dziecka. Co do rasy, nie byłby zaskoczeniem jeżeli wpływ będzie zer-jedynkowy, tj. na wagę wpływać będzie czy dziecko jest białej matki czy czarnej/innej.
```{r include=FALSE}
...
###Analiza wariancji  
Wykonany zostanie test Bartelta.
```{r echo=FALSE}
bartlett.test(split(BWT, list(RACE, SMOKE)))
``` 
Jak widać nie ma podstaw do odrzucenia hipotezy zerowej, tzn że wariancje w grupach są statystycznie różne.  
...
TukeyHSD(fit)
``` 
Jak widać średnie wielokrtnych podgrup nie różnią się istotnie od siebie.
...
```
Wszystkie efekty są na bardzo niskim poziomie- nie przekraczają one 8%.
